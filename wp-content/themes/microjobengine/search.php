			...
			<div class="container">
				<h2 class="block-title">
					<p class="block-title-text" data-prefix="<?php _e('in', 'enginethemes'); ?>">
						...
						if($search_result == 1) {
							printf(__('<span class="search-result-count">%s</span> Search <span class="search-text-result">result</span> for "%s" %s', 'enginethemes'), $search_result, get_query_var('s'), $term_name);
						} else {
							printf(__('<span class="search-result-count">%s</span> Search <span class="search-text-result">results</span> for "%s" %s', 'enginethemes'), $search_result, get_query_var('s'), $term_name);
						}
